## TÉRMINOS Y CONDICIONES DE USO 

**Última actualización:** [13/04/2026]

En cumplimiento con la normativa vigente en España, estos Términos y Condiciones regulan el acceso y uso de la aplicación AURA, titularidad de:

**AURA**    
**Email:** aura.ispp@gmail.com

El acceso y uso de la App atribuye la condición de usuario e implica la aceptación plena de estos términos.

---

## 1. Definiciones

- **AURA**: Plataforma digital que facilita la creación y gestión de tours turísticos.
- **Usuario**: Persona que accede a la App (Guía o Turista).
- **Guía**: Usuario que crea y ofrece tours.
- **Turista**: Usuario que participa en tours.
- **Servicios**: Funcionalidades ofrecidas por AURA.

---

## 2. Objeto del servicio

AURA actúa como **plataforma intermediaria tecnológica** que conecta guías turísticos con usuarios interesados en experiencias turísticas.

AURA no organiza directamente los tours, salvo que se indique expresamente.

---

## 3. Registro y cuentas

- El usuario garantiza que los datos proporcionados son veraces
- Es responsable de mantener la confidencialidad de su cuenta
- Debe ser mayor de 18 años o contar con autorización legal

AURA podrá suspender cuentas en caso de uso indebido.

---

## 4. Responsabilidad de los guías

Los guías son responsables de:

- La ejecución del tour
- La veracidad de la información
- El cumplimiento de normativa local (licencias, seguros, etc.)

AURA no se responsabiliza de:
- Cancelaciones
- Incidencias durante el tour
- Daños derivados de la actividad

---

## 5. Pagos y uso de Stripe

### Procesamiento de pagos

Los pagos realizados en AURA se procesan mediante **Stripe Technology Europe, Ltd.**

Al realizar un pago, el usuario acepta también:
- Los términos de Stripe
- Sus políticas de privacidad

### Modelo de cobro

AURA puede:
- Cobrar comisiones por transacción
- Ofrecer suscripciones a guías
- Cobrar por servicios premium

### Seguridad

- AURA no almacena datos de tarjetas
- Los pagos se procesan en entornos seguros (PCI-DSS)

---

## 6. Uso de Inteligencia Artificial

AURA puede utilizar IA para:
- Recomendaciones
- Recalcular rutas
- Generar contenido informativo

El usuario reconoce que:
- La información puede contener errores
- No sustituye decisiones humanas

---

## 7. Responsabilidad del usuario

El usuario se compromete a:
- Actuar conforme a la ley
- Respetar a otros usuarios
- Cumplir normas de seguridad

AURA no se responsabiliza de:
- Accidentes
- Daños personales
- Pérdidas económicas

---

## 8. Disponibilidad del servicio

AURA no garantiza:
- Funcionamiento ininterrumpido
- Ausencia de errores técnicos

Podrá:
- Suspender temporalmente el servicio
- Realizar mejoras o actualizaciones

---

## 9. Propiedad intelectual

Todos los contenidos de AURA son propiedad de la empresa o cuentan con licencia.

Queda prohibido:
- Copiar
- Distribuir
- Reproducir sin autorización

---

## 10. Protección de datos (RGPD)

AURA cumple con el **Reglamento (UE) 2016/679 (RGPD)** y la **Ley Orgánica 3/2018 (LOPDGDD)**.

Los datos personales serán tratados conforme a la Política de Privacidad.

---

## 11. Limitación de responsabilidades

AURA no será responsable de:
- Daños indirectos
- Lucro cesante
- Fallos derivados de terceros (ej. Stripe, proveedores)

---

## 12. Modificación de términos

AURA podrá modificar estos términos en cualquier momento.

Se notificará a los usuarios mediante:
- App de AURA
- Email

---

## 13. Legislación y Jurisdicción

Estos términos se rigen por la legislación española.

En caso de conflicto, las partes se someten a los tribunales de la ciudad competente.

---

## 14. Contacto

Para cualquier consulta:

Email: aura.ispp@gmail.com 


---

## 15. Aceptación

El uso de la App de AURA implica la aceptación de estos Términos y Condiciones.
